...
// LoadEnvValue reads a single KEY=value line from .env, stripping an inline
// comment and surrounding quotes. The CLI runs as a plain binary and does not
// source .env the way a shell would, so host-side settings kept there (search
// provider keys, for instance) need this to be seen.
func LoadEnvValue(projectRoot, key string) string {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		v = strings.TrimSpace(v)
		v = strings.Trim(v, `"'`)
		value = v // last assignment wins, matching shell semantics
	}
	return value
}
...
// WriteNextAction writes a post-TUI action. target may itself contain '|'
// (e.g. a shell-run command); ReadNextAction preserves everything after the
// first separator.

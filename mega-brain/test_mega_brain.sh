...
# --- Suite 0: detect_project (the real function, before it is stubbed) ---
# This is the logic that used to collapse every project into a shared
# "workspace" bucket. Exercise it via the MB_DETECT_CWD seam.
suite "detect_project"
assert_eq "multi-project path -> project name" \
    "$(MB_DETECT_CWD=/workspace/projects/resume-adapter detect_project)" "resume-adapter"
assert_eq "multi-project subdir -> project name" \
    "$(MB_DETECT_CWD=/workspace/projects/rag-app/src/x detect_project)" "rag-app"
assert_eq "multi-project bare parent -> not a project" \
    "$(MB_DETECT_CWD=/workspace/projects detect_project)" "workspace"
assert_eq "no git + ALCATRAZ_PROJECT_NAME set -> host name" \
    "$(ALCATRAZ_PROJECT_NAME=my-proj MB_DETECT_CWD=/nowhere/xyz detect_project)" "my-proj"
assert_eq "no git + no signal -> workspace fallback" \
    "$(ALCATRAZ_PROJECT_NAME= MB_DETECT_CWD=/nowhere/xyz detect_project)" "workspace"
# A real git repo whose basename is NOT workspace resolves to that basename.
_GITPROJ="$TMPDIR_BRAIN/single-repo/sub"
mkdir -p "$_GITPROJ"
( cd "$TMPDIR_BRAIN/single-repo" && git init -q 2>/dev/null )
assert_eq "single git repo -> repo basename" \
    "$(MB_DETECT_CWD=$_GITPROJ detect_project)" "single-repo"
# Two sibling projects must NOT share a bucket.
_A="$(MB_DETECT_CWD=/workspace/projects/alpha detect_project)"
_B="$(MB_DETECT_CWD=/workspace/projects/beta detect_project)"
if [ "$_A" != "$_B" ]; then _ok "sibling projects get distinct buckets"
else _fail "sibling projects collided -> both '$_A'"; fi
...
# --- Suite 7: cmd_remember (canonical plural dirs) ---
suite "cmd_remember"
_mp="$TMPDIR_BRAIN/testproject"
out_rem="$(cmd_remember "pattern" "Use Composition" "prefer composition" 2>&1)"
assert_file_exists "pattern created"          "$_mp/Memory/patterns/use-composition.md"
assert_file_contains "pattern has type"       "$_mp/Memory/patterns/use-composition.md" "pattern"
assert_file_contains "pattern has date"       "$_mp/Memory/patterns/use-composition.md" "2026-05-30"
assert_file_contains "pattern has project"    "$_mp/Memory/patterns/use-composition.md" "testproject"
assert_contains "output confirms Saved"       "$out_rem" "Saved"
assert_file_contains "INDEX count updated"    "$_mp/INDEX.md" "Patterns: 1 files"

( cmd_remember "decision" "REST over GraphQL" "" 2>/dev/null )
assert_file_exists "decision created" "$_mp/Memory/decisions/rest-over-graphql.md"

( cmd_remember "gotcha" "Null pointer in loop" "" 2>/dev/null )
assert_file_exists "gotcha created" "$_mp/Memory/gotchas/null-pointer-in-loop.md"

( cmd_remember "note" "Deploy on friday" "never" 2>/dev/null )
assert_file_exists "note created" "$_mp/Memory/notes/deploy-on-friday.md"
...
echo "ORIGINAL CONTENT" > "$_mp/Memory/patterns/use-composition.md"
( cmd_remember "pattern" "Use Composition" "new content" 2>/dev/null )
assert_file_contains "existing memory keeps original" \
    "$_mp/Memory/patterns/use-composition.md" "ORIGINAL CONTENT"
assert_file_contains "existing memory gets dated update" \
    "$_mp/Memory/patterns/use-composition.md" "## Update 2026-05-30"
assert_file_contains "update carries new content" \
    "$_mp/Memory/patterns/use-composition.md" "new content"

( cmd_remember "pattern" "Use Composition" "" 2>/dev/null )
out_rem_noc="$(cmd_remember "pattern" "Use Composition" "" 2>&1)"
assert_contains "no content -> no update, just shows" "$out_rem_noc" "Already exists"
...
assert_file_exists "note: learning A" "$_tp/Memory/notes/learning-a.md"
assert_file_exists "note: learning B" "$_tp/Memory/notes/learning-b.md"
...
assert_file_exists "remember creates Memory/patterns itself" \
    "$TMPDIR_BRAIN/proj-no-dirs/Memory/patterns/auto-dir.md"
detect_project() { echo "testproject"; }
...
# --- Suite 15: memory index in loaders ---
suite "memory index"
detect_project() { echo "proj-index"; }
( cmd_remember "gotcha" "Big Table Lock" "ALTER TABLE locks big tables" 2>/dev/null )
out_idx="$(cmd_context_md 2>&1)"
assert_contains "context-md lists memory link" "$out_idx" "[[Memory/gotchas/big-table-lock]]"
assert_contains "context-md lists description" "$out_idx" "ALTER TABLE locks big tables"
assert_contains "context-md explains on-demand load" "$out_idx" "search"
out_load_idx="$(cmd_load 2>&1)"
assert_contains "load lists memory link" "$out_load_idx" "[[Memory/gotchas/big-table-lock]]"
detect_project() { echo "testproject"; }

# --- Suite 16: cmd_search ---
suite "cmd_search"
detect_project() { echo "proj-index"; }
out_search="$(cmd_search "ALTER TABLE" 2>&1)"
assert_contains "search finds project memory" "$out_search" "big-table-lock"
out_search_g="$(cmd_search "nvim" 2>&1)"
assert_contains "search covers global partition" "$out_search_g" "use-nvim"
out_search_none="$(cmd_search "zzz-no-such-term-zzz" 2>&1)"
assert_contains "search without hits says so" "$out_search_none" "(no results)"
( cmd_search "" 2>/dev/null ); rc_search=$?
assert_exit_nonzero "search without term -> exit != 0" "$rc_search"
detect_project() { echo "testproject"; }

# --- Suite 17: cmd_handoff ---
suite "cmd_handoff"
detect_project() { echo "proj-handoff"; }
( cmd_handoff "refactored auth; decided on JWT; next: refresh tokens" 2>/dev/null )
_hp="$TMPDIR_BRAIN/proj-handoff"
assert_file_exists "last-session.md created" "$_hp/Context/last-session.md"
assert_file_contains "handoff has summary"   "$_hp/Context/last-session.md" "next: refresh tokens"
assert_file_contains "timeline records handoff" "$_hp/Logs/timeline.md" "handoff"
out_ho_ctx="$(cmd_context_md 2>&1)"
assert_contains "context-md injects handoff" "$out_ho_ctx" "Last session handoff"
assert_contains "context-md has handoff body" "$out_ho_ctx" "next: refresh tokens"
( cmd_handoff "second session summary" 2>/dev/null )
assert_file_contains "handoff overwritten with latest" "$_hp/Context/last-session.md" "second session"
assert_file_not_contains "old handoff not in last-session" "$_hp/Context/last-session.md" "refresh tokens"
( cmd_handoff "" 2>/dev/null ); rc_ho=$?
assert_exit_nonzero "handoff without summary -> exit != 0" "$rc_ho"
detect_project() { echo "testproject"; }

# --- Suite 18: legacy singular-dir migration ---
suite "legacy migration"
_lp="$TMPDIR_BRAIN/proj-legacy"
mkdir -p "$_lp/Memory/pattern"
echo "# Old Pattern" > "$_lp/Memory/pattern/old-pattern.md"
ensure_structure "$_lp"
assert_file_exists "legacy memory moved to plural dir" "$_lp/Memory/patterns/old-pattern.md"
if [ ! -d "$_lp/Memory/pattern" ]; then _ok "legacy dir removed"
else _fail "legacy dir removed -> Memory/pattern still exists"; fi

# --- Suite 19: cmd_precompact ---
suite "cmd_precompact"
detect_project() { echo "proj-precompact"; }
( cmd_precompact "auto" "- user: fix the login bug
- assistant: done, refactored auth.ts" 2>/dev/null )
_pp="$TMPDIR_BRAIN/proj-precompact"
assert_file_exists "last-session.md created" "$_pp/Context/last-session.md"
assert_file_contains "snapshot labeled pre-compaction" "$_pp/Context/last-session.md" "pre-compaction snapshot"
assert_file_contains "snapshot has trigger" "$_pp/Context/last-session.md" "trigger: auto"
assert_file_contains "snapshot has digest" "$_pp/Context/last-session.md" "refactored auth.ts"
assert_file_contains "timeline records compaction" "$_pp/Logs/timeline.md" "context compacted (auto)"
out_pc_ctx="$(cmd_context_md 2>&1)"
assert_contains "context-md injects snapshot" "$out_pc_ctx" "refactored auth.ts"
( cmd_precompact "manual" "" 2>/dev/null )
assert_file_contains "empty digest -> placeholder" "$_pp/Context/last-session.md" "no digest captured"
assert_file_contains "trigger recorded" "$_pp/Context/last-session.md" "trigger: manual"
detect_project() { echo "testproject"; }

...
build_cli() {
    ...
    # Rebuild when ANY Go source is newer, not just main.go (TUI code lives
    # in pkg/, internal/ — comparing only main.go left stale binaries).
    if [ ! -f "$cli_bin" ] || \

...
test_network() {
    ...
    # HTTP/HTTPS access: only whitelisted domains via proxy (guard -> squid)
...
test_tools() {
    ...
    # Node is the ONLY language runtime the sandbox ships — no stray
    # interpreter should be lying around for a compromised package to use.
    if run_test "No Python" "python3 --version"; then
        test_fail "python3 present — the sandbox should ship Node only"
    else
        test_pass "No stray Python interpreter"
    fi
...
test_proc_masked() {
    ...
    if [ -z "$environ" ] || [ "$environ" -eq 0 ]; then
        ...
        test_pass "/proc/1/environ readable ($environ bytes) - the Guard sanitizes on OUTPUT"
    fi
...
test_env_protection() {
    ...
    # Create a fake .env and try to read it to check the Guard sanitizes
    ...
    if run_test "Read fake .env" "grep SECRET_KEY /tmp/.env.fake"; then
        test_pass "Fake .env readable INSIDE the container (expected - the Guard intercepts on OUTPUT)"
    else
...
test_data_guard_exclusive() {
    echo ""
    echo -e "${YELLOW}=== FAIL-CLOSED TESTS (GUARD) ===${NC}"

    # Check that http_proxy points ONLY to guard
    local proxy_env
    proxy_env=$($DC -f docker-compose.go.yml exec -T alcatraz \
        bash -c 'echo "$http_proxy"' 2>/dev/null || echo "none")
    if echo "$proxy_env" | grep -q "guard"; then
        test_pass "http_proxy points to Guard ($proxy_env)"
    else
        test_fail "http_proxy does NOT point to Guard ($proxy_env)"
    fi

    # Direct egress must be impossible: the sandbox sits on an internal-only
    # network (internal: true), so ignoring the proxy has nowhere to route. If a
    # direct connection to an arbitrary IP succeeds, the network boundary is broken.
    if ! run_test "Proxy bypass (direct IP)" "curl -sfI --noproxy '*' -m 8 https://1.1.1.1 >/dev/null"; then
        test_pass "Direct egress blocked — sandbox has no route out except the proxy (expected)"
    else
        test_fail "Direct IP egress works — the proxy can be bypassed (security failure!)"
    fi
}
...
test_isolation_summary() {
    ...
    echo "  Exfiltration:✓ Blocked (Guard MITM, proxy whitelist, bandwidth limit)"
}
